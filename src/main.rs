...
fn main() {
    ...
    let period = if args.len() > 1 { &args[1] } else { "daily" };
    ...
    match period {
        "weekly" | "monthly" => {
            display_stats(&user_settings, period);
        }
        "daily" | _ => {
            ...
            display_summary(&mut user_settings, &current_time, end_of_day);
            ...
            display_stats(&user_settings, "daily");
        }
    }
}
